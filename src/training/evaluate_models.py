...
def main():
    ...
    jl.Parallel(n_jobs=args.nr_threads)(jl.delayed(evaluation_function)(args, biomarker) for biomarker in biomarkers)

    sort_biomarkers(args, biomarkers)


def evaluate_biomarker_cover(args, biomarker):
...
def evaluate_biomarker_disc(args, biomarker):
    model_file = DataHandler.get_model_file(biomarker)
    eval_file = model_file.replace('.csv', '_eval_{0}.csv'.format(args.metric))
...
def sort_biomarkers(args, biomarkers):
    errors = []
