...
do
  for method in cog mbl long img all
  do
    for visits in "m24" "m12 m24" "bl m12 m24"
    do
      fitting/vgam_predict_biomarker_value.py ${visits} -m ${method} -p "${biomarker}" --exclude_cn --latex_file results.tex --no_plot #--use_last_visit
    done
  done
done
...
do
  ...
  do
    for visits in "bl m12" "bl m12 m24" "m12 m24"
    do
      fitting/vgam_predict_biomarker_value.py ${visits} -m ${method} -p "${biomarker}" --estimate_dpr --exclude_cn --latex_file results.tex --no_plot
    done
  done
done
