...
if [ $experiment -eq "1" ] ; then
  ...
  training/plot_models.py -b "${biomarker}" --no_densities --no_extrapolation --no_sample_lines --no_model --plot_file "${folder}model_generation_align.eps"
  training/plot_models.py -b "${biomarker}" --no_densities --no_extrapolation --no_sample_lines --plot_file "${folder}model_generation_fit.eps"
  training/plot_models.py -b "${biomarker}" --no_densities --plot_file "${folder}model_generation_extrapolate.eps"
  training/plot_models.py -b "${biomarker}" --only_densities --plot_file "${folder}model_generation_densities.eps"
fi
...
if [ $experiment -eq "2" ] ; then
  ...
  do
    biomarker="synth_${b}"
    output_file="${folder}synthetic_model_${b}.eps"
...
if [ $experiment -eq "3" ] ; then
  ...
  do
    output_file="${folder}synthetic_sampling_${sampling}.eps"
...
if [ $experiment -eq "4" ] ; then
  output_file="${folder}eval_synth_num_samples.eps"
...
if [ $experiment -eq "5" ] ; then
  output_file="${folder}eval_synth_estimation.eps"
...
if [ $experiment -eq "6" ] ; then
  ...
  do
    ...
    output_file="${folder}model_${biomarker_str}.eps"
