...
atlas_prefix = os.path.join( atlas_folder, 'model_' + a.viscode + '_' + a.diagnosis + '_s' )

def collect_data( data_folder_adni1, data_folder_adni2, data_folder_adniG,
                   viscode, datafile ):
    
    files, rid, diags, age, mmse = adni.get_all_data( 
    ...
    if os.path.exists( datafile ):
        ...
        with open( datafile, 'wb') as csvfile:
            ...
            for i in range(len( files )):
                ...
                if os.path.exists( files[i] ) and a.diagnosis in diags[i]:
                    virtual_nmi_brain = check_output([execEstimate, files[i], '-mask', mask_brain,
                                                      '-min', '0', '-max', '15', '-stepsize', '0.25',
                                                      '-prefix', atlas_prefix, '-silent' ])
                    csv_writer.writerow([rid[i], diags[i], age[i], mmse[i], virtual_nmi_brain, files[i]])
