...
def main():
    ...
    for name in sorted_metric_names:
        if name in adni.cog_score_names:
            ...
            normalise = True
        analyse_metric(name, name, normalise, plot=True, csv_writer=csv_writer, model=model)
...
def analyse_metric(name_csv, name_hr, normalise, plot=False, csv_writer=None, model=exponential):
